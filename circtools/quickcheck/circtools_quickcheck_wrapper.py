...
def main(args=None):
    ...
    circ_matrix = circ.iloc[:, circ_data_start:].T
    ...
    if do_pca:
        ...
        pca_df = pd.DataFrame(pcs, columns=["PC1", "PC2"], index=samples)
        pca_df["group"] = groups
    ...
    with PdfPages(pdf_name) as pdf:
        ...
        print("Plotting circular vs. linear read counts...")
        ...
        for i, row in df_raw.iterrows():
            plt.text(row["circ"], row["linear"], clean_label(row["sample"]), fontsize=9, ha="left", va="bottom")
        plt.tight_layout()
        pdf.savefig()
        plt.close()

        print("Plotting unique reads vs. detected circRNAs...")
        ...
        print("Plotting circRNAs per million unique reads...")
        ...
        if not df_ratio_clean.empty:
            plt.figure(figsize=(11.69, 8.27))
            order = df_ratio_clean.sort_values("norm")
            plt.bar(order["sample"], order["norm"], color="#3874c8", edgecolor="black")
        ...
        if do_pca:
            print("Plotting PCA of circRNA counts...")
            plt.figure(figsize=(11.69, 8.27))
            sns.scatterplot(data=pca_df, x="PC1", y="PC2", hue="group", style="group", s=100, edgecolor="black")
            for sample, row in pca_df.iterrows():
                plt.text(row["PC1"], row["PC2"], clean_label(sample), fontsize=9, ha="left", va="bottom")
