...
class QuickCheck(circ_module.circ_template.CircTemplate):
    ...
    def run_module(self):
        ...
        import argparse
        import circtools.scripts.circtools_quickcheck_wrapper as wrapper

        wrapper_args = argparse.Namespace(
            dcc_dir=self.cli_params.detect_dir,
            star_dir=self.cli_params.star_dir,
            output_prefix=os.path.join(self.cli_params.output_directory, self.cli_params.output_name),
            conditions=self.cli_params.condition_list,
            grouping=self.cli_params.grouping,
            colour_mode=self.cli_params.colour,
            cleanup_string=self.cli_params.cleanup,
            starfolder_suffix=str(self.cli_params.starfolder),
            remove_suffix_chars=self.cli_params.remove_suffix_chars,
            remove_prefix_chars=self.cli_params.remove_prefix_chars,
            remove_columns=str(self.cli_params.remove_columns),
        )
